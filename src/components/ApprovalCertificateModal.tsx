          ...
          <div>
            <div className="flex items-center gap-2">
              <span className="font-bold text-base md:text-lg text-on-surface">
                Academic Acceptance & Release Sign-Off
              </span>
        ...
        <form onSubmit={handleSignAndIssue} className="space-y-4">
          <div className="p-4 rounded-xl bg-surface-container text-xs text-on-surface space-y-2 border border-outline-variant/20">
            <div className="flex items-center justify-between">
              ...
              <Badge variant="primary">{request.serviceCategory.replace('_', ' ').toUpperCase()}</Badge>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-[11px] pt-1">
              <div>
                <strong>Manuscript Title:</strong> {request.projectTitle}
              </div>
              <div>
                <strong>Author / Scholar:</strong> {request.clientName}
              </div>
              <div>
                <strong>Volume:</strong> {request.estimatedPages} Pages / Slides
              </div>
              <div>
                <strong>Language:</strong> {request.targetLanguage.toUpperCase()}
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <Input
              label="Confirm Scholar Full Name & Title"
              value={scholarName}
            ...
            <Input
              ...
              placeholder="Type your full academic name to sign"
              value={signatureText}
              onChange={(e) => setSignatureText(e.target.value)}
              helperText="Legal electronic authorization for publishing release."
              required
            />
          </div>

          <Textarea
            label="Sign-Off Endorsement Notes"
            rows={2}
            value={approvalNotes}
          ...
          <div className="flex items-center justify-end gap-2.5 pt-3 border-t border-outline-variant/20">
            ...
            <Button type="submit" variant="secondary" className="text-xs font-bold shadow-sm">
              ...
              Execute Sign-off & Issue Certificate
            </Button>
          </div>
        </form>
      ) : (
        ...
        <div className="space-y-4">
          <div className="p-8 sm:p-10 rounded-2xl bg-gradient-to-b from-amber-50/50 via-white to-amber-50/30 dark:from-slate-900 dark:via-slate-800 dark:to-slate-900 border-4 border-double border-amber-600/40 text-on-surface shadow-lg relative overflow-hidden print:border-2 print:p-6">
            ...
            <div className="text-center space-y-1.5 border-b-2 border-amber-600/30 pb-4 relative z-10">
              ...
              <p className="text-xs text-on-surface-variant font-mono">
                Certificate Serial: {certificateData?.certificateNumber} • Issued{' '}
            ...
            <div className="py-6 space-y-4 text-center relative z-10">
              ...
              <p className="text-xs text-on-surface-variant max-w-lg mx-auto leading-relaxed">
                Authored by <strong>{certificateData?.clientName}</strong> (
                {certificateData?.affiliation}), comprising{' '}
                <strong>{certificateData?.pagesApproved}</strong> typeset pages/slides, has
                successfully passed comprehensive editorial copyediting, citation validation, and author
            ...
            <div className="pt-4 mt-4 border-t border-outline-variant/20 flex flex-col sm:flex-row items-center justify-between text-[10px] text-on-surface-variant font-mono">
              ...
              <span>VERIFIED AT HARAMAYA UNIVERSITY PRESS</span>
            </div>
          </div>
          ...
          <div className="flex items-center justify-between pt-2 no-print">
            ...
            <div className="flex items-center gap-2">
              ...
              <a
                href={`${OFFICIAL_BRAND.telegramUrl}?text=Hello%20Mr.%20Feysal,%20I%20have%20signed%20and%20certified%20project%20${request.id}%20(Certificate:%20${certificateData?.certificateNumber})`}
                target="_blank"
                rel="noopener noreferrer"
                className="px-4 py-2 rounded-xl bg-[#0088cc] hover:bg-[#0077b5] text-white text-xs font-bold flex items-center gap-1.5 shadow-xs transition-colors"
              >
